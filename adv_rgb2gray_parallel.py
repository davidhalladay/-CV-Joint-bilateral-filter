...
class adv_rgb2gray:
    ...
    def run_sigle(self, scores, img_rgb, sigma_s, sigma_r ,parallel_num):
        cost = []
        ...
        for idx, point in enumerate(tqdm(self.points,desc = "[s:{}, r:{}]".format(sigma_s,sigma_r),position = parallel_num)):
    ...
    def run_base(self, path, output_path, parallel_num):
        ...
        for s in self.sigma_s:
            for r in self.sigma_r:
                scores = self.run_sigle(scores, img_rgb, s, r, parallel_num)
        ...
        return weight_score_dict

    def run_parallel(self, img_path_list, output_path):
        num_cores = multiprocessing.cpu_count()
        #self.run_base(img_path_list[2], output_path, parallel_num = 0)
        results = Parallel(n_jobs = num_cores)(delayed(self.run_base)(path, output_path, idx) \
...
def main():
    ...
    converter.run_parallel(img_path_list, args.output_path)
...
def test():
    ...
    results_score = [[9,9,0],
                     [3,3,2],
                     [3,3,3]]
    def base(img_id , img_path):
        for idx, (w_list, score_list) in enumerate(zip(results_w,results_score)):
            for j in range(3):
                myscore = converter.verify(img_path, sigma_s_l, sigma_r_l, w_list[j])
                print("Image {}-{} get score {} ......{}".format(img_id+1,idx+1,
                        myscore, (myscore == score_list[j])))

    num_cores = multiprocessing.cpu_count()
    results = Parallel(n_jobs = num_cores)(delayed(base)(img_id, img_path) \
